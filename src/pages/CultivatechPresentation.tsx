...
import { ArrowLeft, Sprout, Layers, Gem, BarChart as BarChartIcon, Users, PieChart as PieChartIcon, TrendingUp, Scale, Bot, DollarSign, Mail } from "lucide-react";
                            ...
                            <Card className="tech-border hover-lift group overflow-hidden relative flex flex-col">
                                {slide.image && (
                                    <div className="relative h-64 md:h-80 overflow-hidden">
                                        <img src={slide.image} alt={slide.imageAlt || slide.title} className={cn("w-full h-full object-cover group-hover:scale-110 transition-transform duration-500")} />
                                        <div className="absolute inset-0 bg-gradient-to-t from-background via-background/60 to-transparent" />
                                    </div>
                                )}
                                <CardHeader className="text-center pt-8">
                                    <div className="flex items-center justify-center mb-4">
                                        {slide.icon && <slide.icon className="h-12 w-12 mr-4 text-primary" />}
								...
								<CardContent className="p-6 md:p-8 flex-grow">
                                        ...
                                        <div className="space-y-5 text-lg text-muted-foreground max-w-prose mx-auto lg:mx-0">
                                            {Array.isArray(slide.content)
                                                ? slide.content.map((item, i) => (
                                                    <p key={i} dangerouslySetInnerHTML={{ __html: item.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>') }} />
                                                ))
                                                : slide.content && (
                                                    <p dangerouslySetInnerHTML={{ __html: slide.content.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>') }} />
                                                )
                                            }
                                        </div>
                                                ...
                                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full max-w-lg">
                                                    {slide.materials.map((material) => (
                                                        <div key={material.name} className="p-3 rounded-lg border-2 border-black/10 dark:border-white/10 flex flex-col justify-between aspect-square" style={{ backgroundColor: material.color, boxShadow: `0 0 25px -5px ${material.color}` }}>
                                                            <span className="text-2xl font-bold text-black/80">{material.atomicNumber}</span>
                                                            <h3 className="text-6xl font-black text-center text-black/90" style={{ textShadow: '1px 1px 2px rgba(255,255,255,0.2)'}}>{material.symbol}</h3>
                                    ...
                                    {slide.source && (
                                        <p className="text-xs text-muted-foreground/70 italic text-center mt-8 pt-4 border-t border-border/50">{slide.source}</p>
                                    )}
								</CardContent>
